...
//didn't understand shit about font and text falan filan
    ...
    font = TTF_OpenFont("vga857.ttf", 1000);
...
//ball hitting sides
    ...
    else if(ball.y>=WINDOW_HEIGHT-ball.height){ball.y=WINDOW_HEIGHT-ball.height;ball.speedy*=-1;}
//score
    if(ball.x>WINDOW_WIDTH+5){ball.x=WINDOW_HEIGHT/2; ball.y=WINDOW_WIDTH/2; ball_hit=0; ball.speedx=BALL_SPEED_X; ball.speedx*=-1; player2.score++;}
    if(ball.x<0){ball.x=WINDOW_HEIGHT/2; ball.y=WINDOW_WIDTH/2; ball_hit=0; ball.speedx=BALL_SPEED_X; ball.speedx*=-1; player1.score++;}
